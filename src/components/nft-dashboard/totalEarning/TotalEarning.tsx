...
import { TotalEarningChart } from 'components/nft-dashboard/totalEarning/TotalEarningChart/TotalEarningChart';
import { useAppSelector } from 'hooks/reduxHooks';
import {
  getTotalEarning,
  TotalEarning as ITotalEarning,
} from 'apis/earnings.api';
...
export const TotalEarning: React.FC = () => {
  ...
  useEffect(() => {
    userId &&
      getTotalEarning(userId, CurrencyTypeEnum.USD).then((res) =>
        setTotalEarning(res),
      );
  }, [userId]);
  ...
  const { totalEarningData, days } = useMemo(
    () => ({
      totalEarningData: {
        data: totalEarning
          ? totalEarning.timeline.map((item) => item.usd_value)
          : [],
      },
      days: totalEarning
        ? totalEarning.timeline.map((item) =>
            Dates.getDate(item.date).format('L'),
          )
        : [],
    }),
    [totalEarning],
  ...
  const isIncreased =
    Number(totalEarning?.total) > Number(totalEarning?.prevTotal);
          ...
          <BaseRow wrap={false} justify="space-between">
            ...
            <BaseCol>
              <S.ValueText $color={isIncreased ? 'success' : 'error'}>
                ...
                {totalEarning &&
                  getDifference(totalEarning?.total, totalEarning?.prevTotal)}
              </S.ValueText>
            </BaseCol>
          </BaseRow>
        </BaseCol>
        ...
        <BaseCol span={24}>
          <BaseRow wrap={false} justify="space-between" gutter={[20, 20]}>
            <BaseCol>
              <S.Text>
                {getCurrencyPrice(
                  formatNumberWithCommas(totalEarning?.total ?? 0),
