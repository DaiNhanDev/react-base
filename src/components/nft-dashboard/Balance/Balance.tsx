...
import { Balance as IBalance, getBalance } from 'apis/earnings.api';
...
export const Balance: React.FC = () => {
  ...
  const onFinish = (values: TopUpData) => {
    ...
    setTimeout(() => {
      setBalance((balance) => ({
        ...balance,
        [values.currency]: balance[values.currency] + values.amount,
      }));
      setLoading(false);
      setModalOpen(false);
    }, 1000);
              ...
              <BaseRow gutter={[14, 14]}>
                <BaseCol span={24}>
                  <S.TitleBalanceText level={3}>
                    {getCurrencyPrice(
                      formatNumberWithCommas(balance.USD),
                  ...
                  <BaseRow gutter={[55, 10]} wrap={false}>
                    <BaseCol>
                      <S.SubtitleBalanceText>
                        {getCurrencyPrice(
                          formatNumberWithCommas(balance.ETH),
                    ...
                    <BaseCol>
                      <S.SubtitleBalanceText>
                        {getCurrencyPrice(
                          formatNumberWithCommas(balance.BTC),
