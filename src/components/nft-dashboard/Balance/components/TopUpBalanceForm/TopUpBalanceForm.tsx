...
import { getCurrencyPrice, getPaymentCardTypeIcon } from 'utils/utils';
...
export const TopUpBalanceForm: React.FC<TopUpDataProps> = ({
  cards,
  loading,
  onFinish,
}) => {
          ...
          <NFTCard>
              ...
              <InputNumber
                ...
                addonBefore={
                  <BaseForm.Item name="currency" noStyle initialValue="USD">
                    <BaseSelect>
                      {(
                        Object.keys(CurrencyTypeEnum) as Array<
                          keyof typeof CurrencyTypeEnum
                        >
                      ).map((type) => (
            ...
            <BaseForm.Item
              shouldUpdate={(prev, cur) =>
                (prev as CurrencyValues).currency !==
                (cur as CurrencyValues).currency
              }
              noStyle
            >
              {({ getFieldValue }) => {
                  ...
                  <S.ButtonsWrapper>
                    {array.map((amount) => (
                      <BaseButton
                        ...
                        onClick={() =>
                          form.setFieldsValue({
                            amount: amountFieldValue + amount,
                          })
                        }
                      >
