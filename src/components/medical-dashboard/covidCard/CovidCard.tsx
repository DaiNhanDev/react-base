...
import { getCovidData, CoronaData } from 'apis/covid.api';
...
export const CovidCard: React.FC = () => {
  ...
  const { confirmedArr, deathsArr, dateArr } = useMemo(() => {
    const confirmedArr: number[] = Object.values(data?.cases || {}).splice(
      0,
      100,
    );
    const deathsArr: number[] = Object.values(data?.deaths || {}).splice(
      0,
      100,
    );
